...
const METRICS: &[Metric] = &[
    Metric { value: "50K+", label: "Revshare NFTs Minted" },
    Metric { value: "7", label: "Production dApps" },
    Metric { value: "WCHL2025", label: "Hackathon Winner" },
    Metric { value: "100%", label: "Fair Launched" },
];
...
fn TypewriterText() -> impl IntoView {
    ...
    // Run the typewriter animation in a spawned task
    spawn_local(async move {
        ...
        loop {
            ...
            // Type the word
            for i in 1..=current_word.len() {
                set_display_text.set(current_word[..i].to_string());
            ...
            // Delete the word
            for i in (0..current_word.len()).rev() {
                set_display_text.set(current_word[..i].to_string());
...
fn Hero() -> impl IntoView {
    view! {
        <section class="hero">
            <TypewriterText />
            <p class="subtitle">"Building ICP's Great City"</p>
        </section>
    }
}

#[component]
fn Metrics() -> impl IntoView {
    view! {
        <section class="metrics">
            {METRICS.iter().map(|m| view! {
                <div class="metric">
...
#[component]
fn ProductRow(product: &'static Product) -> impl IntoView {
    view! {
        <a
            href={product.url}
            class="table-row"
        ...
        >
            <div class="col-project">
                ...
                <div class="tags">
                    {product.tags.iter().map(|tag| view! {
                        <span class="tag">{*tag}</span>
                    }).collect_view()}
                </div>
            </div>
            <div class="col-tagline">{product.tagline}</div>
